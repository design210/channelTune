...
export default {
  ...
  watch: {
    ...
    channeltune2Files(data) {
      this.$emit("uploadFiles2", data);
    },
    //파일 리사이징
    channeltune2FilesOrigin(file) {
      const uploadfileLen = this.channeltune2Files.length;
      ...
      if (uploadfileLen < len) {
            ...
            new Compressor(ele, {
              ...
              success(result) {
                ...
                if (fileSize.length === len) {
                  _this.channeltune2Files = fileSize;
                  bus.$emit("end:spinnerBg");
                }
              },
              error(err) {
        ...
        if (this.deleteFile !== "") {
          ...
          const newFile = [];
          this.channeltune2Files.forEach((ele) => {
            if (ele.name !== delfile) {
              newFile.push(ele);
            }
          });
          this.channeltune2Files = newFile;
        }
        if (this.channeltune2FilesOrigin < 1) {
          this.channeltune2Files = [];
        }
      }
    },
  ...
  methods: {
    //파일 첨부
    selectFile(file) {
      this.channeltune2FilesOrigin = file;
      if (file.length > 0) {
        this.addClass = true;
      } else {
        this.addClass = false;
    ...
    deleteChip(index, file) {
      this.channeltune2FilesOrigin.splice(index, 1);
      this.deleteFile = file;
    },
    deleteAllChip(index) {
      if (this.channeltune2FilesOrigin.length > 0) {
        this.channeltune2FilesOrigin.splice(index);
        this.channeltune2Files = [];
      }
    },
    //용량체크 알림
