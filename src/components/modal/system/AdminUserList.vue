...
import AdminPersonListModal from "@/components/modal/system/AdminList";
...
export default {
  ...
  methods: {
    ...
    //회사 모달
    showModalPopup() {
      this.$modal.show(AdminPersonListModal, { update: this.update }, getPopupOpt("AdminPersonListModal", "700px", "auto", false));
    },
    update(data) {
      this.sid = data.adminUserSid;
      this.name = data.adminUserName;
    },
    levelProp(data) {
    ...
    async confirm() {
      ...
      try {
        ...
        if (this.setAllPowerResult.channeltuneApiResult.errorCode == 200) {
          this.closeModal = true;
          this.alim("등록 되었습니다.", this.successColor);
        } else {
          this.alim(this.setAllPowerResult.channeltuneApiResult.errorMessage, this.errorColor);
        }
      } catch (error) {
