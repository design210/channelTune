  ...
  methods: {
    async reload() {
      try {
        ...
        if (this.getAdminList.channeltuneApiResult.errorCode == 200) {
          this.userList = this.getAdminList.channeltuneApiResult.menuMemberList;
        } else {
          this.alim(this.getAdminList.channeltuneApiResult.errorMessage, this.errorColor);
        }
      } catch (error) {
    ...
    async userDetail(id) {
      try {
        ...
        if (this.getAdminDetail.channeltuneApiResult.errorCode == 200) {
          this.userId = id;
          //총 갯수
          const count = Object.keys(this.getAdminDetail.channeltuneApiResult.menuMemberList).length;
          //새로운 배열
          const list = [];
          for (let i = 0; i < count; i++) {
            await list.push(this.getAdminDetail.channeltuneApiResult.menuMemberList[i]);
          }
          ...
          this.alim(this.getAdminDetail.channeltuneApiResult.errorMessage, this.errorColor);
        }
      } catch (error) {
        this.alim(error, this.errorColor);
      } finally {
      ...
      if (data.del === "Y") {
        if (data.type === "list") {
          try {
            ...
            if (this.powerAdminDeleteResult.channeltuneApiResult.errorCode == 200) {
              this.reload();
              this.menuDataList = [];
              this.alim("삭제 되었습니다.", this.successColor);
            } else {
              this.alim(this.powerAdminDeleteResult.channeltuneApiResult.errorMessage, this.errorColor);
            }
          } catch (error) {
            this.alim(error, this.errorColor);
          } finally {
