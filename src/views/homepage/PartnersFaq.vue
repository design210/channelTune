      ...
      <section class="member-list section-box border-outside">
        <div class="border">
          ...
          <table class="tbl mt20 user-list-tbl">
            ...
            <tbody>
              <tr v-for="(item, index) in list" :key="index" @click="detail(item.boardFaqSid)">
                <td>{{ item.boardFaqSid }}</td>
                <td class="left subject">{{ item.boardFaqTitle }}</td>
...
export default {
  ...
  computed: {
    ...mapGetters("common", ["partnerFaqList"]),
  },
  watch: {
    page() {
      this.reload();
  ...
  methods: {
    async reload() {
      try {
        ...
        await this.$store.dispatch("common/PARTNER_FAQ_LIST", {
          size: this.size,
          page: this.page,
          searchText: this.searchText,
        });
        const res = this.partnerFaqList.channeltuneApiResult;
        if (parseInt(res.errorCode) !== 200) {
          this.alim(res.errorMessage, this.errorColor);
          return false;
        }
        const data = res.boardPartnerFaq;
        this.list = data.data;
    ...
    detail(id) {
      this.$router.push(`/homepage/partners/faq/view/${id}`);
    },
    write() {
      this.$router.push("/homepage/partners/faq/write/");
    },
  },
};
</script>
