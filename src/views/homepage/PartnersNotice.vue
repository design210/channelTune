      ...
      <section class="member-list section-box border-outside">
        <div class="border">
          ...
          <table class="tbl mt20 user-list-tbl">
            ...
            <tbody>
              <tr v-for="(item, index) in list" :key="index" @click="detail(item.boardNoticeSid)">
                <td>{{ item.boardNoticeSid }}</td>
                <td class="left subject">{{ item.boardNoticeTitle }}</td>
...
export default {
  ...
  computed: {
    ...mapGetters("common", ["partnerNoticeList"]),
  },
  watch: {
    page() {
      this.reload();
  ...
  methods: {
    async reload() {
      try {
        ...
        await this.$store.dispatch("common/PARTNER_NOTICE_LIST", {
          size: this.size,
          page: this.page,
          searchText: this.searchText,
        });
        const res = this.partnerNoticeList.channeltuneApiResult;
        if (parseInt(res.errorCode) !== 200) {
          this.alim(res.errorMessage, this.errorColor);
          return false;
        }

        const data = res.boardPartnerNotice;
        this.list = data.data;
    ...
    detail(id) {
      this.$router.push(`/homepage/partners/notice/view/${id}`);
    },
    write() {
      this.$router.push("/homepage/partners/notice/write/");
    },
  },
};
</script>
