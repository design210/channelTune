...
export default {
  ...
  computed: {
    ...mapGetters("common", ["partnerFaqDetail", "partnerFaqModifyResult", "partnerFaqDelResult"]),
  ...
  methods: {
    ...
    async reload() {
      ...
      try {
        ...
        await this.$store.dispatch("common/PARTNER_FAQ_DETAIL", id);
        const res = this.partnerFaqDetail.channeltuneApiResult;
        ...
        const detail = res.boardPartnerFaq;
    ...
    async save() {
      ...
      try {
        ...
        await this.$store.dispatch("common/PARTNER_FAQ_MODIFY", {
          ...
          boardFaqGroupSort: this.boardFaqGroupNo,
          boardFaqGroupDepth: this.boardFaqGroupSort,
          channeltuneFiles: this.channeltuneFiles,
          boardCategorySids: this.category,
        });

        const res = this.partnerFaqModifyResult.channeltuneApiResult;
        if (parseInt(res.errorCode) !== 200) {
    ...
    async emitResetConfirm(data) {
      if (data.del === "Y") {
        if (data.type === "list") {
          try {
            bus.$emit("start:spinner");
            await this.$store.dispatch("common/PARTNER_FAQ_DEL", this.boardFaqSid);
            const res = this.partnerFaqDelResult.channeltuneApiResult;
            if (parseInt(res.errorCode) !== 200) {
              this.alim(res.errorMessage, this.errorColor);
              return false;
            }

            await this.$router.push("/homepage/partners/faq");
          } catch (error) {
            this.alim(error, this.errorColor);
          } finally {
            bus.$emit("end:spinner");
